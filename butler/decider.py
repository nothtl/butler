...
# is answered live by the LLM from live state via Decider._do_chat / Chat.converse.
...
class Decider:
    ...
    def parse(self, message: str) -> Intent:
        ...
        # Unmatched free text -> live LLM chat (never a canned help/reply).
        return Intent("chat", raw=msg)
    ...
    def resolve(self, intent: Intent, user: str = "user") -> Any:
        ...
        if k == "ask":
            ...
            if not q:
                return {"kind": "chat", "mode": "ask", "answer": self._live_context()}
            return {"kind": "chat", "mode": "ask",
                    "answer": self.chat.answer(q, live_context=self._live_context())}
        if k == "teach":
            q = (intent.query or "").strip()
            if not q:
                return {"kind": "help"}
    ...
    def _do_chat(self, intent: Intent) -> str:
        """Live conversational reply for free-text that matched nothing else.

        No canned text: the LLM answers from live state gathered now. If the LLM
        is unavailable we return the live state itself (data), never an invented
        sentence. See AGENTS.md ("no hardcoded replies").
        """
        msg = (intent.raw or intent.query or "").strip()
        live = self._live_context()
        if not msg:
            return live
        reply = self.chat.converse(msg, live_context=live)
        return reply or live

    def _live_context(self) -> str:
        """Compact, current snapshot of everything Butler actually knows.

        Passed verbatim to the LLM so it can answer questions about its own
        access/capabilities from real data instead of guessing.
        """
        cfg = self.cfg
        lines: list[str] = []
        try:
            ...
            if connected:
                cal_id = GoogleCalendar(cfg).calendar_id()
                lines.append(f"Google Calendar: CONNECTED (active calendar id: {cal_id})")
            else:
                lines.append("Google Calendar: NOT connected")
        except Exception as exc:  # noqa: BLE001
            lines.append(f"Google Calendar: status unavailable ({exc})")
        snap = self._snapshot()
        events = snap.get("events_today", []) or []
        if events:
            ev = "; ".join(f"{e.get('title', 'event')} "
                           f"@ {e.get('start', e.get('start_ts', ''))}" for e in events[:10])
            lines.append(f"Events today ({len(events)}): {ev}")
        else:
            lines.append("Events today: none")
        tasks = snap.get("tasks", []) or []
        lines.append(f"Active tasks: {len(tasks)}"
                     + (": " + "; ".join(str(t.get('title', '')) for t in tasks[:10]) if tasks else ""))
        courses = snap.get("courses", []) or []
        lines.append(f"Courses tracked: {len(courses)}"
                     + (": " + "; ".join(str(c.get('code', '')) for c in courses[:10]) if courses else ""))
        lines.append(f"Indexed files: {snap.get('file_count', 0)}")
        lines.append(f"Course documents indexed: {snap.get('courses_document_count', 0)}")
        pres = snap.get("presence", {}) or {}
        lines.append(f"Presence: {'known' if pres.get('known') else 'unknown'}"
                     + (f" ({pres.get('status')}/{pres.get('zone')})" if pres.get('known') else ""))
        lines.append(f"Current time: {self._now_human()}")
        return "\n".join(lines)
